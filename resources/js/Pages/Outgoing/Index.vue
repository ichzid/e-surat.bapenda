...
watch(search, debounce(function (value) {
    router.get(route('outgoing.index'), { search: value }, { 
        preserveState: true, 
        replace: true 
    });
}, 300));
...
const submit = () => {
    ...
    if (isEditing.value) {
        form.transform((data) => ({
            ...data,
            _method: 'put',
        })).post(route('outgoing.update', form.id), {
            forceFormData: true,
...
const deleteDocument = (doc) => {
    if (confirm(`Hapus data surat keluar no: ${doc.document_number}?`)) {
        router.delete(route('outgoing.destroy', doc.id));
    }
};
    ...
    <AuthenticatedLayout>
        <div class="max-w-7xl mx-auto w-full">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between mt-2 mb-8 gap-4">
                <div>
                    <h1 class="font-display text-3xl font-bold text-navy">Surat Keluar</h1>
                    <p class="font-sans text-slate-secondary mt-1">Daftar arsip dokumen dan surat yang dikirimkan oleh instansi.</p>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-slate-200 bg-slate-50">
                                ...
                                <th class="py-3 px-4 text-xs font-bold text-navy uppercase tracking-wider w-1/4">Tujuan / Penerima</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr v-if="documents.data.length === 0">
                                <td colspan="5" class="py-8 text-center text-sm text-slate-500">Tidak ada data surat keluar ditemukan.</td>
                            </tr>
                            <tr v-for="doc in documents.data" :key="doc.id" class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="py-3 px-4">
                                    <div class="text-xs">
                                        ...
                                        <span class="text-slate-500 block mt-0.5">Dikirim: <span class="font-medium text-navy">{{ formatDate(doc.received_date) }}</span></span>
                                    </div>
                                </td>
            ...
            <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 border-b border-slate-100">
                    ...
                    <form @submit.prevent="submit" class="space-y-4">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Tujuan/Penerima Surat</label>
                                <input v-model="form.sender_or_receiver" type="text" class="w-full border border-slate-200 rounded-lg p-2.5 text-sm focus:border-sage focus:ring-sage" placeholder="Cth: Dinas Kesehatan Provinsi" required>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Tanggal Dikirim</label>
                ...
                <div class="bg-slate-50 px-4 py-3 sm:px-6 flex flex-row-reverse gap-3">
                    <button @click="submit" :disabled="form.processing" type="button" class="w-full inline-flex justify-center rounded-lg border border-transparent px-4 py-2 bg-sage text-base font-medium text-white shadow-sm hover:bg-sage/90 focus:outline-none sm:w-auto sm:text-sm">
                        ...
                        <span v-else>{{ isEditing ? 'Simpan Perubahan' : 'Upload Surat Keluar' }}</span>
                    </button>
